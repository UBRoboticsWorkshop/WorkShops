...
1. Press the Start button on you PCs and open up 'NI VirtualBench'
...
2. It'll then prompt you to search or connect to a device. There should only be one device available, select it. The application should then take you to a screen like the one shown above.
...
3. Press the default button located in the top left corner of the screen. This will clear up a previously set configuration.
...
#### Next, we’re going to generate a PWM signal

- A PWM signal is essentially a square wave signal that has 2 defining parameters, duty cycle and frequency.
...
Connect the grounds (alligator clips) of the Function Generator and Channel 1 probes and then connect the probes themselves to one another.
...
6. Change the trigger level around until the wave becomes stationary
...
Undo the connections that you made in the 'Let’s get started’ section.
...
11. Compare with the people around you. Are there any differences in the motor’s behaviour? Why?
...
The load pin can be pulled either low or high by activating its gate, labelled Low/High Switch. This allows for each side of the motor to be disconnected, low or high.
